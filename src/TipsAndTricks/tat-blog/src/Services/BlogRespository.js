...
export function getPosts(
  ...
  return get_api(
    `https://localhost:7085/api/posts?keyword=${keyword}&PageSize=${pageSize}&PageNumber=${pageNumber}&SortColumn=${sortColumn}&SortOrder=${sortOrder}`
  );
}
export function getAuthors(
  ...
  return get_api(
    `https://localhost:7085/api/authors?name=${name}&PageSize=${pageSize}&PageNumber=${pageNumber}&SortColumn=${sortColumn}&SortOrder=${sortOrder}`
  );
}
...
export async function getPostById(id = 0) {
  if (id > 0) return get_api(`https://localhost:7085/api/posts/${id}`);
  return null;
}
